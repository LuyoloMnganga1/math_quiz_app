...
import 'Third_grade_scoreboard.dart';

class ThirdGradeMultiplyTest extends StatefulWidget {
  final int? score;
  final int? wrong;
  const ThirdGradeMultiplyTest({Key? key, @required this.score, this.wrong})
      : super(key: key);

  @override
  _ThirdGradeMultiplyTestState createState() => _ThirdGradeMultiplyTestState();
}

class _ThirdGradeMultiplyTestState extends State<ThirdGradeMultiplyTest> {
  late int _score;
  late int _wrong;
  List<int> question = RandomMultiplyIntGenerator(1);
  ...
  void nextQuestionCorrect(BuildContext context) {
    ...
    Future.delayed(Duration(seconds: 1), () {
      Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(
              builder: (context) => ThirdGradeMultiplyTest(
                    score: _score,
                    wrong: _wrong,
                  )),
          (route) => false);
