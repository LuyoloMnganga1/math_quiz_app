...
import 'package:math_quiz_app/screens/first_grade/first_grade_scoreboard.dart';

class FirstGradeSumXTest extends StatefulWidget {
  final int? score;
  final int? wrong;
  const FirstGradeSumXTest({Key? key, @required this.score, this.wrong})
      : super(key: key);

  @override
  _FirstGradeSumXTestState createState() => _FirstGradeSumXTestState();
}

class _FirstGradeSumXTestState extends State<FirstGradeSumXTest> {
  late int _score;
  late int _wrong;
  List<int> question = RandomAlgGenerator(1);
  late int num1 = question[0];
  late String num2 = "X";
  late int num3   = question[1];
  late var answers = RandomSumAnswerGeneratorX(num1, num3);
  ...
  void navigateToScoreboard(BuildContext context) {
    Future.delayed(Duration(seconds: 1), () {
      Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) =>
                FirstGradeScoreBoard(score: _score, level: 1)),
      );
    });
  }
