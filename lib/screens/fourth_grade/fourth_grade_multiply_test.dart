...
class FourthGradeMultiplyTest extends StatefulWidget {
  final int? score;
  final int? wrong;
  const FourthGradeMultiplyTest({Key? key, @required this.score, this.wrong})
      : super(key: key);

  @override
  _FourthGradeMultiplyTestState createState() =>
      _FourthGradeMultiplyTestState();
}

class _FourthGradeMultiplyTestState extends State<FourthGradeMultiplyTest> {
  late int _score;
  late int _wrong;
  List<int> question = RandomMultiplyIntGenerator(2);
  ...
  void nextQuestionCorrect(BuildContext context) {
    ...
    Future.delayed(Duration(seconds: 1), () {
      Navigator.pushAndRemoveUntil(
          context,
          MaterialPageRoute(
              builder: (context) => FourthGradeMultiplyTest(
                    score: _score,
                    wrong: _wrong,
                  )),
          (route) => false);
