...
import 'package:math_quiz_app/screens/second_grade/second_grade_scoreboard.dart';

class SecondGradeSumTest2 extends StatefulWidget {
  final int? score;
  final int? wrong;
  const SecondGradeSumTest2({Key? key, @required this.score, this.wrong})
      : super(key: key);

  @override
  _SecondGradeSumTestState createState() => _SecondGradeSumTestState();
}

class _SecondGradeSumTestState extends State<SecondGradeSumTest2> {
  late int _score;
  late int _wrong;
  List<int> question = RandomSumIntGenerator(2);
  late int num1 = question[0];
  late int num2 = question[1];
  late var answers = RandomSumAnswerGenerator(num1, num2);
  ...
  void navigateToScoreboard(BuildContext context) {
    Future.delayed(Duration(seconds: 1), () {
      Navigator.push(
        context,
        MaterialPageRoute(
            builder: (context) => SecondGradeScoreboard(
                  score: _score,
                  level: 2,
                  method: Methods.sum,
                )),
